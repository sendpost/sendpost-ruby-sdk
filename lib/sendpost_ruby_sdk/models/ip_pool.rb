...
module Sendpost
  class IPPool
    ...
    attr_accessor :ip_domain_warmup_status
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Sendpost::IPPool` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Sendpost::IPPool`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'routing_meta_data')
        self.routing_meta_data = attributes[:'routing_meta_data']
      end

      if attributes.key?(:'auto_warmup_enabled')
        self.auto_warmup_enabled = attributes[:'auto_warmup_enabled']
      end
      ...
      if attributes.key?(:'ip_domain_warmup_status')
        self.ip_domain_warmup_status = attributes[:'ip_domain_warmup_status']
      end

      if attributes.key?(:'should_overflow')
        self.should_overflow = attributes[:'should_overflow']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          ip_domain_warmup_status == o.ip_domain_warmup_status &&
          should_overflow == o.should_overflow &&
