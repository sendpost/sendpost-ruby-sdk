  ...
  class IPUpdateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Sendpost::IPUpdateRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Sendpost::IPUpdateRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @auto_warmup_enabled.nil?
        invalid_properties.push('invalid value for "auto_warmup_enabled", auto_warmup_enabled cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @auto_warmup_enabled.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] auto_warmup_enabled Value to be assigned
    def auto_warmup_enabled=(auto_warmup_enabled)
      if auto_warmup_enabled.nil?
        fail ArgumentError, 'auto_warmup_enabled cannot be nil'
      end
