...
module Sendpost
  class CreateSuppressionRequest
    ...
    def self.openapi_types
      {
        :'hard_bounce' => :'Array<CreateSuppressionRequestHardBounceInner>',
        :'manual' => :'Array<CreateSuppressionRequestManualInner>',
        :'unsubscribe' => :'Array<CreateSuppressionRequestUnsubscribeInner>',
        :'spam_complaint' => :'Array<CreateSuppressionRequestSpamComplaintInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Sendpost::CreateSuppressionRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Sendpost::CreateSuppressionRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [hard_bounce, manual, unsubscribe, spam_complaint].hash
    end
