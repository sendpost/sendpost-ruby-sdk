  ...
  class IPAllocationRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Sendpost::IPAllocationRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Sendpost::IPAllocationRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @overflow_pool.nil?
        invalid_properties.push('invalid value for "overflow_pool", overflow_pool cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @overflow_pool.nil?
      return false if @ips.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] overflow_pool Value to be assigned
    def overflow_pool=(overflow_pool)
      if overflow_pool.nil?
        fail ArgumentError, 'overflow_pool cannot be nil'
      end
