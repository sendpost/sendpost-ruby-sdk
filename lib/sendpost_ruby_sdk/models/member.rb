  ...
  class Member
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Sendpost::Member` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Sendpost::Member`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'is_forbidden')
        self.is_forbidden = attributes[:'is_forbidden']
      end

      if attributes.key?(:'firebase_uid')
        self.firebase_uid = attributes[:'firebase_uid']
      end
      ...
      if attributes.key?(:'onboard_q_answered')
        self.onboard_q_answered = attributes[:'onboard_q_answered']
      end
      ...
      if attributes.key?(:'notes_color')
        self.notes_color = attributes[:'notes_color']
      end
    ...
    def hash
      [id, is_verified, is_forbidden, firebase_uid, email, name, url, company_name, onboard_q_answered, phone_number, notes_color, created].hash
    end
