  ...
  class ThirdPartySendingProvider
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Sendpost::ThirdPartySendingProvider` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Sendpost::ThirdPartySendingProvider`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'retry_time')
        self.retry_time = attributes[:'retry_time']
      end
    ...
    def hash
      [id, name, type, domain, endpoint, key, secret, port, oauth_token, retry_time, created].hash
    end
