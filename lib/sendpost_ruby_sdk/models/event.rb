  ...
  class Event
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Sendpost::Event` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Sendpost::Event`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'message_subject')
        self.message_subject = attributes[:'message_subject']
      end
    ...
    def hash
      [event_id, groups, ip_id, ip_pool_id, domain_id, tpsp_id, message_type, message_subject, account_id, sub_account_id, message_id, type, from, from_name, to, to_name, submitted_at, smtp_code, smtp_description, event_metadata].hash
    end
