  ...
  class Webhook
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Sendpost::Webhook` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Sendpost::Webhook`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'unique_open')
        self.unique_open = attributes[:'unique_open']
      end
    ...
    def hash
      [id, enabled, url, processed, delivered, dropped, soft_bounced, hard_bounced, opened, clicked, unsubscribed, spam, sent, smtp_dropped, unique_open, unique_click, created, created_by, updated_by].hash
    end
